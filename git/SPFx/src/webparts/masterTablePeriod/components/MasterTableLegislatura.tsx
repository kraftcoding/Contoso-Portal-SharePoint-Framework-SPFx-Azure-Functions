...
import styles from './MasterTablePeriod.module.scss';
import * as strings from 'MasterTablePeriodWebPartStrings';
import { IMasterTablePeriodprops } from './IMasterTablePeriodprops';
import { IPeriodItem, PeriodRepository } from './MasterTablePeriodModels';
...
const fromDateInput = (value: string): Date | undefined => (value ? new Date(value + 'T00:00:00') : undefined);

const normalizeRange = (start?: Date, end?: Date) => {
  const s = start ? start.getTime() : Number.NEGATIVE_INFINITY;
  const e = end ? end.getTime() : Number.POSITIVE_INFINITY;
  return [s, e] as const;
};
...
const rangesOverlap = (
  ...
  return s1 <= e2 && s2 <= e1; // inclusivo
};


/*---- Componente ----*/
const MasterTablePeriod: React.FC<IMasterTablePeriodprops> = ({ context, title, pageSize }) => {

  /*---- Repositorio y constantes ---- */
  const repo = React.useMemo(() => new PeriodRepository(context), [context]);

  /*---- Estados ----*/
  const [items, setItems] = React.useState<IPeriodItem[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [formErrors, setFormErrors] = React.useState<{ codigo?: string; descripcion?: string, StartDate?:string, EndDate?:string, fechaGeneral?:string }>({});
  ...
  const onSave = async (): promise<void> => {
    const errors: { codigo?: string; descripcion?: string, StartDate?:string, EndDate?:string, fechaGeneral?:string } = {};

    if (!form.codigo?.trim()) {
      errors.codigo = strings.RequiredFieldError;
    }
    if (!form.descripcion?.trim()) {
      errors.descripcion = strings.RequiredFieldError;
    }

    if (!form.StartDate) errors.StartDate = strings.StartDateRequiredError;
 
    // Validación de coherencia de fechas (si ambas están informadas)
    if (form.StartDate && form.EndDate && form.StartDate > form.EndDate) {
      errors.fechaGeneral = strings.DateEndGtStartError;
    }

    if(form.StartDate){
      // --- Comprodbación de solapamiento en cliente
      const currentId = form.id;
      const conflicts = items.filter(i => {
        if (currentId && i.Id === currentId) return false; // excluir el prodpio en edición
        return rangesOverlap(form.StartDate, form.EndDate, i.StartDate, i.EndDate);
      });

      if (conflicts.length > 0) {
        const conflictList = conflicts
          .map(c => `• ${c.descripcion ?? c.codigo ?? `#${c.Id}`}`)
          .join('\n');
        errors.fechaGeneral = `${strings.DateOverlapError}\n${conflictList}`
      }
    }
  ...
  const confirmDelete = React.useCallback(async (): promise<void> => {
    ...
    try {
          ...
          setError(null);
          //Dejo comentada comprodbacion a futuro sobre el posible campo de lookup de Period
          // const currentId = confirmState.ids[0];
          // const hasReferences = await repo.checkLookupFieldsInUse(currentId, "PeriodLookup",context.pageContext.site.serverRelativeUrl + "/DepartmentsEXTERNAL" )
          ...
          switch (e.status) {
          case 403:
                message= strings.NoPermissionError;
                break;
          case 404:
                message= strings.NotFoundError;
                break;
          case 500:
                if (/restrict delete/i.test(e.message) || /relacionado/i.test(e.message)) {
                message= strings.ReferencedError;
                } else {
                message=strings.ErrorMsg;
                }
                break;
          default:
                message=`Error (${e.status}): intenta más tarde.`;
          }
  ...
  const desktopColumns = React.useMemo<TableColumnDefinition<IPeriodItem>[]>(() => [
    createTableColumn<IPeriodItem>({
      columnId: 'codigo',
      compare: (a, b) => (a.codigo ?? '').localeCompare(b.codigo ?? ''),
      renderHeaderCell: () => strings.codigo,
    ...
    createTableColumn<IPeriodItem>({
      columnId: 'StartDate',
      compare: (a, b) => (a.StartDate?.getTime() ?? 0) - (b.StartDate?.getTime() ?? 0),
      renderHeaderCell: () => strings.StartDate,
      renderCell: item =>
        item.StartDate
          ? item.StartDate.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
          : '',
    }),
    createTableColumn<IPeriodItem>({
      columnId: 'EndDate',
      compare: (a, b) => (a.EndDate?.getTime() ?? 0) - (b.EndDate?.getTime() ?? 0),
      renderHeaderCell: () => strings.EndDate,
      renderCell: item =>
        item.EndDate
          ? item.EndDate.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
          : '',
    }),
    createTableColumn<IPeriodItem>({
      columnId: 'activo',
        ...
        <div className={styles.actionsCell}>
          ...
          <Tooltip content={strings.Delete} relationship="label">
            <Button
              ...
              onClick={e => { e.stopprodpagation(); deleteById(item.Id, item.descripcion ?? item.codigo ?? `#${item.Id}`); }}
            />
          </Tooltip>
        </div>
      ),
  ...
  const mobileColumns = React.useMemo<TableColumnDefinition<IPeriodItem>[]>(() => [
    createTableColumn<IPeriodItem>({
      columnId: 'descripcion',
      compare: (a, b) => (a.descripcion ?? '').localeCompare(b.descripcion ?? ''),
      renderHeaderCell: () => strings.descripcion,
            ...
            <MenuPopover>
              <MenuList>
                ...
                <MenuItem
                  ...
                  onClick={e => {
                    e.stopprodpagation();
                    deleteById(item.Id, item.descripcion ?? item.codigo ?? `#${item.Id}`);
                  }}
                >
                  {strings.Delete}
                </MenuItem>
    ...
    <div className={styles.wrapper}>
      ...
      {!loading && (
        <>
          ...
          >
            ...
            <DataGridBody<IPeriodItem>>
              {({ item }) => (
                <DataGridRow<IPeriodItem> key={item.Id}>
      ...
      >
        <DialogSurface aria-describedby={descId} className={styles.mobileDialogSurface}>
          ...
          <DialogActions className={styles.dialogActionsStack}>
            <Button
              onClick={() => {setConfirmState((s) => ({ ...s, open: false }));setError(null);setFormErrors({})}}
      ...
      <Dialog open={openForm} onOpenChange={(_, data) => setOpenForm(data.open)}>
        <DialogSurface className={styles.mobileDialogSurface}>
          ...
          <DialogBody className={styles.dialogBodyFix}>
            <div className={styles.formGrid}>
              <Field
                ...
                required
                validationMessage={formErrors.codigo}
                validationState={formErrors.codigo ? 'error' : 'none'}
              >
                <Input
                  value={form.codigo}
                  onChange={(_, d) => setForm(f => ({ ...f, codigo: d.value }))}
                />
              </Field>
              ...
              <Field label={strings.StartDate} required
                validationMessage={formErrors.StartDate}
                validationState={formErrors.StartDate ? 'error' : 'none'}>
                <input
                  className={styles.dateInput}
                  type="date" //para que ponga un placeholder tipo "desde" hay que cambiar a datepicker
                  value={toDateInput(form.StartDate)}
                  onChange={(e) => setForm(f => ({ ...f, StartDate: fromDateInput(e.target.value) }))}
                />
              </Field>
              <Field label={strings.EndDate}
                validationMessage={formErrors.EndDate}
                validationState={formErrors.EndDate ? 'error' : 'none'}
              >
                <input
                  className={styles.dateInput}
                  type="date" //para que ponga un placeholder tipo "hasta" hay que cambiar a datepicker
              ...
              <Field
                style={{ gridColumn: '1 / -1' }}
                validationMessage={formErrors.fechaGeneral}
                validationState={formErrors.fechaGeneral ? 'error' : 'none'}
              ></Field>
              <Field>
          ...
          <DialogActions>
            <Button onClick={() => {setOpenForm(false); setFormErrors({})}}>{strings.Cancel}</Button>
...
export default MasterTablePeriod;