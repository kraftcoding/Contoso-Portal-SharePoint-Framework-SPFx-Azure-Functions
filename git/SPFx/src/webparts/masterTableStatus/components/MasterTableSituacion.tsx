...
import styles from './MasterTableStatus.module.scss';
import * as strings from 'MasterTableStatusWebPartStrings';
import { IMasterTableStatusprops } from './IMasterTableStatusprops';
import { IStatusItem, StatusRepository } from './MasterTableStatusModels';
...
/*---- Componente ----*/
const MasterTableStatus: React.FC<IMasterTableStatusprops> = ({ context, title, pageSize }) => {

  /*---- Repositorio y constantes ---- */
  const repo = React.useMemo(() => new StatusRepository(context), [context]);

  /*---- Estados ----*/
  const [items, setItems] = React.useState<IStatusItem[]>([]);
  ...
  const confirmDelete = React.useCallback(async (): promise<void> => {
    ...
    try {
      ...
      const hasReferences = await repo.checkLookupFieldsInUse(currentId, "StatusLookup",context.pageContext.site.serverRelativeUrl + "/DepartmentsEXTERNAL" )
      ...
      if(!hasReferences){
        ...
        setError("No se puede eliminar: el objeto se está utilizando en al menos un órgano de EXTERNAL.");
      }
    } catch (e: unknown) {
      let message ="";
  ...
  const desktopColumns = React.useMemo<TableColumnDefinition<IStatusItem>[]>(() => [
    createTableColumn<IStatusItem>({
  ...
  const mobileColumns = React.useMemo<TableColumnDefinition<IStatusItem>[]>(() => [
    createTableColumn<IStatusItem>({
      columnId: 'descripcion',
      compare: (a, b) => (a.descripcion ?? '').localeCompare(b.descripcion ?? ''),
      renderHeaderCell: () => strings.descripcion,
      ...
      {!loading && (
        <>
          ...
          >
            ...
            <DataGridBody<IStatusItem>>
              {({ item }) => (
                <DataGridRow<IStatusItem> key={item.Id}>
...
export default MasterTableStatus;