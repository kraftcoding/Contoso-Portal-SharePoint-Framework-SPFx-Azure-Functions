...
export class StatusRepository {
  ...
  private toModel = (r: RawItem): IStatusItem => ({
    Id: r.Id,
    title: r.Title,
  ...
  private toTitleFromDescripcion(data: Partial<IStatusItem>): string | undefined {
    const t = (data.descripcion ?? '').trim();
    return t ? t.substring(0, 255) : undefined; // SharePoint Title máx. 255 chars
  }

  public async getAll(): promise<IStatusItem[]> {
  ...
  public async add(data: Partial<IStatusItem>): promise<number> {
  ...
  public async update(id: number, data: Partial<IStatusItem>): promise<void> {
