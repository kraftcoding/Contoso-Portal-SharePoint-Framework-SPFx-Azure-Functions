...
import styles from './MasterTableTipoDocument.module.scss';
import * as strings from 'MasterTableTipoDocumentWebPartStrings';
import { IMasterTableTipoDocumentprops } from './IMasterTableTipoDocumentprops';
import { ITipoDocumentItem, TipoDocumentRepository } from './MasterTableTipoDocumentModels';
...
/*---- Componente ----*/
const MasterTableTipoDocument: React.FC<IMasterTableTipoDocumentprops> = ({ context, title, pageSize }) => {

  /*---- Repositorio y constantes ---- */
  const repo = React.useMemo(() => new TipoDocumentRepository(context), [context]);

  /*---- Estados ----*/
  const [items, setItems] = React.useState<ITipoDocumentItem[]>([]);
  ...
  const confirmDelete = React.useCallback(async (): promise<void> => {
    ...
    try {
      ...
      const hasReferences = await repo.checkLookupFieldsInUse(currentId, "TipoDocumentLookup",context.pageContext.site.serverRelativeUrl + "/DepartmentsEXTERNAL" )
      ...
      if(!hasReferences){
        ...
        setError("No se puede eliminar: el objeto se está utilizando en al menos un Document de EXTERNAL.");
      }
    } catch (e: unknown) {
      let message ="";
  ...
  const desktopColumns = React.useMemo<TableColumnDefinition<ITipoDocumentItem>[]>(() => [
    createTableColumn<ITipoDocumentItem>({
  ...
  const mobileColumns = React.useMemo<TableColumnDefinition<ITipoDocumentItem>[]>(() => [
    createTableColumn<ITipoDocumentItem>({
      columnId: 'descripcion',
      compare: (a, b) => (a.descripcion ?? '').localeCompare(b.descripcion ?? ''),
      renderHeaderCell: () => strings.descripcion,
      ...
      {!loading && (
        <>
          ...
          >
            ...
            <DataGridBody<ITipoDocumentItem>>
              {({ item }) => (
                <DataGridRow<ITipoDocumentItem> key={item.Id}>
...
export default MasterTableTipoDocument;