...
import styles from './MasterTableDivision.module.scss';
import * as strings from 'MasterTableDivisionWebPartStrings';
import { IMasterTableDivisionprops } from './IMasterTableDivisionprops';
import { IDivisionItem, DivisionRepository } from './MasterTableDivisionModels';
...
/*---- Componente ----*/
const MasterTableDivision: React.FC<IMasterTableDivisionprops> = ({ context, title, pageSize }) => {

  /*---- Repositorio y constantes ---- */
  const repo = React.useMemo(() => new DivisionRepository(context), [context]);

  /*---- Estados ----*/
  const [items, setItems] = React.useState<IDivisionItem[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [formErrors, setFormErrors] = React.useState<{ codigo?: string; descripcion?: string }>({});
  ...
  const onSave = async (): promise<void> => {
    const errors: { codigo?: string; descripcion?: string } = {};
  ...
  const confirmDelete = React.useCallback(async (): promise<void> => {
    ...
    try {
      ...
      const hasReferences = await repo.checkLookupFieldsInUse(currentId, "DivisionLookup",context.pageContext.site.serverRelativeUrl + "/Lists/BusinessAreaDivision")
      if(!hasReferences){
        await promise.all(confirmState.ids.map(id => repo.remove(id)));
        setConfirmState({ open: false, ids: [] });
        await load();
      }else{
        setError("No se puede eliminar: el objeto se está utilizando en una relación Area sectorial - Division.");
      }
    } catch (e: unknown) {
      let message ="";
  ...
  const desktopColumns = React.useMemo<TableColumnDefinition<IDivisionItem>[]>(() => [
    createTableColumn<IDivisionItem>({
      columnId: 'codigo',
  ...
  const mobileColumns = React.useMemo<TableColumnDefinition<IDivisionItem>[]>(() => [
    createTableColumn<IDivisionItem>({
      columnId: 'descripcion',
      compare: (a, b) => (a.descripcion ?? '').localeCompare(b.descripcion ?? ''),
      renderHeaderCell: () => strings.descripcion,
  ...
  const columnSizingOptions = React.useMemo(
    () => ({
      descripcion: {
        ...
        defaultWidth: isMobile ? 360 : 700, // 70%
      },
      codigo: {
        minWidth: isMobile ? 80 : 100,
        defaultWidth: isMobile ? 100 : 200, // 20%
      },
      activo: {
      ...
      {!loading && (
        <>
          ...
          >
            ...
            <DataGridBody<IDivisionItem>>
              {({ item }) => (
                <DataGridRow<IDivisionItem> key={item.Id}>
...
export default MasterTableDivision;