...
# Base variables
$siteURL = "https://contoso-dev.sharepoint.com/sites/epl-test-002/"
$siteColumn = "GrupoguestsAsociado"
$reportOnly = $false # If $true, just report. If $false, take action.

# Connect to the tenant
$siteConnection = Connect-PnPOnline -Url $siteUrl -DeviceLogin -ReturnConnection
...
$cts = Get-PnPContentType -Connection $siteConnection | Where-Object { $_.Group -eq "service-account@contoso.local" }

foreach ($ct in $cts) {

    Write-Host "Checking Content Type $($ct.Name)"

    $fields = Get-PnPproperty -ClientObject $ct -property "Fields" -Connection $siteConnection | Where-Object { $_.InternalName -eq $siteColumn }
...
foreach ($list in $lists) {
    ...
    $field = Get-PnPField -List $list -Connection $siteConnection | Where-Object { $_.InternalName -eq $siteColumn }
...
if (!$reportOnly) {
    Remove-PnPField -Identity $siteColumn -Connection $siteConnection
}